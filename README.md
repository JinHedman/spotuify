...
- **Playbar** — three rows: an 8x3 cover thumbnail, then title / artist / album·year in their own column, with elapsed-total, progress bar, and shuffle-repeat-volume grouped on the right. Costs one row from the content area above compared with the old two-row layout. The thumbnail is too small to recognise a cover but carries the album's colour; it stays silent when there is no art rather than showing a placeholder.
